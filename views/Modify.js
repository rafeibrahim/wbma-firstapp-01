...
import {Container, Content, Text, Form, Item, Button, Badge} from 'native-base';
...
const Modify = (props) => {
  ...
  const modifyAsync = async () => {
    ...
    if (titleOK) {
      try {
        handleModify(props.navigation, props.navigation.state.params.fileData.file_id);
      } catch (e) {
  ...
  return (
    <Container>
      <Content>
        <Form>
          <Text style={{textAlign: 'center', fontWeight: 'bold'}}>Modify</Text>
          <Item>
            <FormTextInput
              value={inputs.title}
              placeholder='title'
          ...
          {(errors.title) &&
            <Badge style={{width: '100%'}}><Text>Title cannot be less than three characters!!</Text></Badge>
          }
        </Form>
      </Content>
    </Container>
